...
description: When the same multi-step workflow repeats in Cursor (user corrections or agent redos) — or recurs across the durable learnings in context/ — capture it as a new SKILL.md under .cursor/skills/ so future sessions load it automatically.
user-invocable: true
---
...
- **Reviewing persisted context** (e.g. at Phase 8 retro, or periodically): a learning recurs across the durable memory written by `saving-workspace-context`.

## Inputs: live repetition *and* persisted context

A pattern can surface two ways:

1. **Live repetition** — you notice the same steps within or across sessions (the triggers above).
2. **Persisted context** — mine the durable memory in `context/`:
   - Read `context/retros/*.md` and the dated entries in `context/<topic>.md`.
   - Cluster learnings that recur **across features** — a single feature rarely repeats 3×, so cross-feature memory is the stronger signal than in-session counting.
   - Triage each recurring learning **by type before promoting** (see the table below):
     - procedure with steps → **skill** (continue with the workflow below),
     - always-on convention → **rule** via `suggesting-cursor-rules`,
     - "on save/stop" automation → **hook** in `.cursor/hooks.json`.
   - **Propose candidates to the user — don't auto-create** skills or rules; they affect every future session (per `saving-workspace-context`).
