...
- Verify tests, types, and lint pass — run the commands documented in [AGENTS.md](../../../AGENTS.md) for each stack you touched (this repo: backend `pytest` + `ruff check .`; frontend `npm test` + `npm run lint`). `.cursor/hooks/check-tests.sh` runs both test suites.
