...
3. **Fix CI failures** — for each failing check, get the logs:
   ...
   Use the commands documented in [AGENTS.md](../../../AGENTS.md) for the affected stack (this repo: backend `pytest` / `ruff`; frontend `npm test` / `npm run lint` / `npm run build`).

   - **Lint failures**: run the linter locally (backend `ruff check . --fix`; frontend `npm run lint -- --fix`), fix remaining issues manually, commit.
   - **Type errors**: run the type-checker (frontend `npm run lint`, i.e. `tsc --noEmit`), read the errors, fix the types, commit.
   - **Test failures**: run the failing suite locally (`pytest` or `npm test`), read the assertion errors, fix the code or update the test expectations, commit.
   - **Build failures**: run the build (frontend `npm run build`), read the error output, fix imports/configs/missing deps, commit.
