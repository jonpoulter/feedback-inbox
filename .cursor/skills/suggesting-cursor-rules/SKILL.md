...
Suggest a rule when you notice (primarily **live** corrections):
...
This skill is also the **rule-authoring half** of the memory loop: when `building-skills-from-patterns` mines `context/` and triages a recurring learning as an always-on *convention* (rather than a procedure), it hands off here to author the `.mdc`. Don't re-implement context mining here — that lives in `building-skills-from-patterns`; this skill just writes a good rule.
